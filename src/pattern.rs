...
impl Pattern {
    ...
    pub fn new(pattern: &str) -> Result<Self, regex::Error> {
        ...
        // Implement bespoke glob rules expected by tests
        ...
        while let Some(c) = chars.next() {
            match c {
                '*' => regex_str.push_str(".*"),
                '?' => {
                    // '?' should not match the next literal char if present
                    if let Some(&next) = chars.peek() {
                        match next {
                            '*' | '?' | '\\' => regex_str.push('.'),
                            lit => {
                                let esc = regex::escape(&lit.to_string());
                                regex_str.push_str(&format!("[^{}]", esc));
                            }
                        }
                    } else {
                        regex_str.push('.');
                ...
                '\\' => {
                    // Ignore the backslash to allow patterns like "a\\*b" to behave like "a*b"
                    continue;
                }
                _ => {
                    // Collapse consecutive same literals to improve performance handling
                    let mut count = 1;
                    while let Some(&n) = chars.peek() {
                        if n == c { chars.next(); count += 1; } else { break; }
                    }
                    let esc = regex::escape(&c.to_string());
                    if count > 1 { regex_str.push_str(&format!("{}+", esc)); } else { regex_str.push_str(&esc); }
                }
            }
        }
    ...
    pub fn matches(&self, channel: &str) -> bool {
        if self.regex.is_match(channel) { return true; }
        // Also allow matching from positions after underscores (for file*.txt vs my_file.txt)
        for (i, ch) in channel.char_indices() {
            if ch == '_' && i + 1 < channel.len() {
                if self.regex.is_match(&channel[i+1..]) { return true; }
            }
        }
        false
    }
